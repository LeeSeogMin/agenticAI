...
LightRAG 저자들의 벤치마크에 따르면, Naive RAG 및 GraphRAG 대비 쿼리 지연이 감소하고 답변 품질이 향상되었다. 특히 인덱싱 비용이 낮아 빠른 프로토타이핑에 적합하다.
...
벡터 RAG는 특정 정보를 검색하는 단순 질문에 강하다. "asyncio.gather()의 매개변수는?"처럼 하나의 청크에서 답을 찾을 수 있는 경우 빠르고 정확한 결과를 반환한다. 실시간 응답이 필요하거나 인덱싱 비용을 최소화해야 할 때도 벡터 RAG가 적합하다. 문서가 자주 업데이트되는 환경에서는 재인덱싱 비용이 낮은 벡터 RAG가 유리하다.

### 그래프 RAG가 필요한 경우

그래프 RAG는 "전체 데이터의 주요 테마는 무엇인가?"처럼 전역적 질문에 강점을 보인다. 엔티티 간 관계를 파악해야 하거나 다중 홉 추론이 필요한 복잡한 질문에서 벡터 RAG보다 우수한 답변을 생성한다. 의료, 법률, 금융 등 도메인 지식의 구조화가 중요한 분야에서는 지식 그래프가 기존 온톨로지와 자연스럽게 결합할 수 있다.

### 하이브리드 접근

실무에서는 벡터 검색과 그래프 검색을 결합하는 것이 효과적이다. 벡터 검색으로 관련 청크를 먼저 찾고, 그래프 순회로 관련 엔티티를 확장하여 맥락을 보강하는 방식이다. LightRAG의 듀얼 레벨 검색이 이러한 하이브리드 접근의 대표적 예다.
...
10장에서는 에이전트 보안과 신뢰성을 다룬다. OWASP AI Agent Security Top 10에 기반한 보안 위협 분류, 다층 가드레일 아키텍처, 에이전트 평가 프레임워크를 학습하고, 검증 실패 시 재시도와 사람 승인이 포함된 워크플로우를 구현한다.
