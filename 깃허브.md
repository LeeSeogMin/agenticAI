...
- 승인 후에도 바로 안 보일 때 → 5~30분 기다리거나 로그아웃 후 재로그인
- "무료 버튼이 안 보임" → 캐시 지우기 / 다른 브라우저 시도 / https://github.com/settings/copilot 직접 들어가기
...
6. **문제 해결 (사용자 과거 이슈: OpenClaw M1, bkit 스킬 반영)**
...
**객관적 판단**: MCP는 Copilot을 2-5배 생산성 향상(외부 컨텍스트 액세스)이지만, 학생 quota 소모/버그로 실무(GeoAI, 연구계획서)에서 GPT 모델 우선. 한국 대학(경기대 등) 재학증명으로 Pro 유지 시 최적. 공식 문서: https://docs.github.com/ko/copilot/how-tos/provide-context/use-mcp. 추가 서버(예: NRRF 데이터 MCP) 필요 시 Registry 검색 권장 – 추측 없음, 모든 내용 2026.2.6 검색 결과 기반.
...
6. **실제 사용 방법** (Copilot Chat에서)
    ...
    예시 프롬프트들 (skills 자동 적용됨):
    
    - "이 함수에 대한 unit test를 작성해줘"
    - "login 컴포넌트에 Jest 테스트 케이스 만들어줘"
    - "pytest로 edge case 테스트 추가해"
    
    → Copilot이 프롬프트 + [SKILL.md](http://skill.md/) 내용을 보고 자동으로 해당 skill 로드 → 지침 엄격 준수
...
- quota 빨리 소진 → 복잡한 skill은 한 번에 여러 파일 생성 → premium request 과다 사용
- 개인 전체 적용하고 싶음 → ~/.copilot/skills/ 또는 ~/.claude/skills/ 에 동일 구조 생성 (Copilot CLI·coding agent 지원)

**객관적 평가**:

Skills는 MCP보다 설정이 훨씬 쉽고, 프로젝트·팀 내 일관된 코딩 규칙 강제에 매우 효과적입니다.

하지만 외부 시스템 연동(이슈 생성, DB 조회 등)은 여전히 MCP가 우위입니다.

두 기능을 함께 쓰는 것이 2026년 현재 가장 생산성 높은 조합으로 평가됩니다.

필요하면 특정 작업(예: GeoAI 파이프라인 테스트 자동화)에 최적화된 [SKILL.md](http://skill.md/) 예시도 추가로 제공 가능합니다.

# 웹 프로그래밍(frontend/backend/full-stack)에서 GitHub Copilot **MCP 서버**와 **Agent Skills**

2026년 2월 기준으로 실제 개발자 커뮤니티(레딧, GitHub, Microsoft 블로그 등)에서 가장 자주 언급되고 유용하다고 평가되는 것들 위주로 정리했습니다.
