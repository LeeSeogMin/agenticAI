...
- **MCP**
  ...
  - 질문: "무엇을 호출할 수 있는가?"
- **Skills / Instructions**
  - 에이전트가 작업할 때 따라야 할 절차와 규칙
  - 질문: "어떻게 일하게 할 것인가?"
- **Plugins / Apps / Connectors**
  - 제품 안에서 기능을 배포하거나 연결하는 방식
  - 질문: "이 기능을 어떤 제품 표면에 어떻게 붙일 것인가?"

- 이 세 개는 이름이 비슷해 보여도 층위가 다름
- 학생이 흔히 하는 오해:
  - MCP = 모든 확장 기능
  - Skill = 프롬프트 한 줄
  - Plugin = 표준 프로토콜
...
- 주의
  ...
  - 따라서 강의실 환경이 VS Code 안정판만 사용한다면 Skills 실습은 Copilot CLI 또는 Insiders 기준으로 안내하는 것이 안전함
...
1. `agent-starter/` 디렉토리를 만든다
2. 가상환경 `.venv`를 생성하고 활성화한다
3. 기본 디렉토리 `code/`, `output/`, `logs/`, `docs/`를 만든다
4. `.env.example`와 `requirements.txt`를 만든다
...
```text
이 프로젝트를 에이전트 실습용 starter 구조로 정리해줘.
다음 조건을 지켜줘:
- code, output, logs, docs 디렉토리를 만든다
- .env.example, requirements.txt, checklist.md를 만든다
- output/hello.txt를 생성하는 최소 실행 스크립트도 작성한다
